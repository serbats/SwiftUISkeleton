...
public struct SkeletonForEach<Data, Content>: View where Data : RandomAccessCollection, Content: View {
    let data: Data
    private let quantity: Int
    let content: (Data.Element?) -> Content
    
    public init(_ data: Data, quantity: Int = 1, @ViewBuilder content: @escaping (Data.Element?) -> Content) {
        self.data = data
